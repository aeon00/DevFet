...
# Load mesh
mesh = sio.load_mesh('/envau/work/meca/data/Fetus/datasets/MarsFet/output/svrtk_BOUNTI/output_BOUNTI_surfaces/haste/sub-0858_ses-0995_reo-SVR-output-brain-mask-brain_bounti-white.left.surf.gii')
vertices = mesh.vertices
...
for i in range(5000, 10001, 500):
    ...
    colors = ['blue', 'green', 'red', 'yellow']  # Colors for B4, B5, B6, B7
    cmap = ListedColormap(colors)
    
    # Plot available bands from B4-B6
    plot_data = np.copy(loc_dom_band)
    # Get available bands between 4-6
    available_bands = [b for b in [4, 5, 6, 7] if b <= np.max(loc_dom_band)]
    ...
    plt.title('Local Dominant Bands (B4-B7)')
    plt.colorbar(ticks=[4, 5, 6, 7], label='Band')
    plt.axis('equal')
    ...
    N_values.append(i)
    for idx, band in enumerate(['B4', 'B5', 'B6', 'B7']):
