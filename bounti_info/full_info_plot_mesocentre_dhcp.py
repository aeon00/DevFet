...
def process_single_file(filename, surface_path, df):
    ...
    try:
        ...
        tex_path = f"/scratch/hdienye/dhcp_full_info/spangy/textures/spangy_dom_band_{participant_session}.gii"
...
def main():
    try:
        ...
        ensure_dir_exists('/scratch/hdienye/dhcp_full_info/principal_curv_tex/')
        ensure_dir_exists('/scratch/hdienye/dhcp_full_info/mean_curv_tex/')
        ensure_dir_exists('/scratch/hdienye/dhcp_full_info/spangy/plots/')
        ensure_dir_exists('/scratch/hdienye/dhcp_full_info/spangy/textures/')
        ensure_dir_exists('/scratch/hdienye/dhcp_full_info/info/')
        ...
        if 'SLURM_ARRAY_TASK_ID' not in os.environ:
            ...
            if results:
                ...
                output_file = os.path.join('/scratch/hdienye/dhcp_full_info/info/', 'all_results.csv')
        ...
        if results:
            results_df = pd.DataFrame(results)
            chunk_file = os.path.join('/scratch/hdienye/dhcp_full_info/info/', 'chunk_{}_results.csv'.format(task_id))
